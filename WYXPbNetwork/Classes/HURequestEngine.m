...
@property (nonatomic, assign) HURequestSerializerType baseRequestSerializer;
@property (nonatomic, assign) HUResponseSerializerType baseResponseSerializer;
@property (nonatomic, assign) BOOL consoleLog;
@end

@implementation HURequestEngine{
    NSMutableDictionary * _requestDic;
}

+ (instancetype)defaultEngine{
    static HURequestEngine *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[HURequestEngine alloc]init];
    });
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self.responseContentTypes addObjectsFromArray:@[@"text/html",@"application/json",@"text/json", @"text/plain",@"text/javascript",@"text/xml",@"image/*",@"multipart/form-data",@"application/octet-stream",@"application/zip",@"application/x-protobuf"]];
...
- (NSUInteger)dataTaskWithMethod:(HUURLRequest *)request
...
#pragma mark - upload
- (NSUInteger)uploadWithRequest:(HUURLRequest *)request
    ...
    NSURLSessionDataTask *uploadTask = [self POST:[NSString hu_stringUTF8Encoding:request.url] parameters:request.parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        
        [request.uploadDatas enumerateObjectsUsingBlock:^(HUUploadData *obj, NSUInteger idx, BOOL *stop) {
            if (obj.fileData) {
...
- (NSUInteger)downloadWithRequest:(HUURLRequest *)request resumeData:(NSData *)resumeData savePath:(NSString *)savePath progress:(void (^)(NSProgress *downloadProgress)) downloadProgressBlock completionHandler:(void (^)(NSURLResponse *response, NSURL *filePath, NSError *error))completionHandler{
    ...
    NSURLRequest *urlRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString hu_stringUTF8Encoding:request.url]]];
...
//请求参数的格式
- (void)requestSerializerConfig:(HUURLRequest *)request{
    self.requestSerializer =request.requestSerializer==HUHTTPRequestSerializer ?[AFHTTPRequestSerializer serializer]:[AFJSONRequestSerializer serializer];
}

//请求头设置
- (void)headersAndTimeConfig:(HUURLRequest *)request{
...
//        request.apiType=HURequestTypeKeepFirst;
//    }
//    if (request.methodType==HUMethodTypeDownLoad) {
//        request.apiType=HURequestTypeKeepFirst;
//    }
    //=====================================================
...
- (void)printParameterWithRequest:(HUURLRequest *)request{
    if (request.consoleLog==YES) {
        NSString *requestStr=request.requestSerializer==HUHTTPRequestSerializer ?@"HTTP":@"JOSN";
        NSString *responseStr=request.responseSerializer==HUHTTPResponseSerializer ?@"HTTP":@"JOSN";
        NSLog(@"\n------------HUNetworking------request info------begin------\n-URLAddress-: %@ \n-parameters-:%@ \n-Header-: %@\n-userInfo-: %@\n-timeout-:%.2f\n-requestSerializer-:%@\n-responseSerializer-:%@\n------------HUNetworking------request info-------end-------",request.url,request.parameters, self.requestSerializer.HTTPRequestHeaders,request.userInfo,self.requestSerializer.timeoutInterval,requestStr,responseStr);
    }
}

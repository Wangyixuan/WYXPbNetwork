...
NSString *const hu_PathSpace =@"HUKit";
NSString *const hu_defaultCachePathName =@"AppCache";
static const NSInteger hu_defaultCacheMaxCacheAge  = 60*60*24*7;
static const CGFloat unit = 1000.0;

@interface HUCacheManager ()
@property (nonatomic ,strong) NSCache *memoryCache;
@property (nonatomic ,copy)NSString *diskCachePath;
@property (nonatomic ,strong) dispatch_queue_t operationQueue;
@end

@implementation HUCacheManager

+ (HUCacheManager *)sharedInstance{
    static HUCacheManager *cacheInstance=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cacheInstance = [[HUCacheManager alloc] init];
    });
...
- (id)init{
    self = [super init];
    if (self) {
        NSString *memoryNameSpace = [@"memory.HUCacheManager" stringByAppendingString:hu_defaultCachePathName];
        
        _operationQueue = dispatch_queue_create("dispatch.HUCacheManager", DISPATCH_QUEUE_SERIAL);
        
        _memoryCache = [[NSCache alloc] init];
        _memoryCache.name = memoryNameSpace;
        
        [self initCachesfileWithName:hu_defaultCachePathName];
...
- (NSString *)HUKitPath{
    return [[self cachesPath]stringByAppendingPathComponent:hu_PathSpace];
}

- (NSString *)HUAppCachePath{
...
- (void)initCachesfileWithName:(NSString *)name{
    self.diskCachePath =[[self HUKitPath] stringByAppendingPathComponent:name];
...
#pragma  mark - 存储
- (void)storeContent:(NSObject *)content forKey:(NSString *)key isSuccess:(HUCacheIsSuccessBlock)isSuccess{
    [self storeContent:content forKey:key inPath:self.diskCachePath isSuccess:isSuccess];
}

- (void)storeContent:(NSObject *)content forKey:(NSString *)key inPath:(NSString *)path isSuccess:(HUCacheIsSuccessBlock)isSuccess{
    if (!content || !key) {
        if (isSuccess) {
...
- (void)getCacheDataForKey:(NSString *)key value:(HUCacheValueBlock)value{
    [self getCacheDataForKey:key inPath:self.diskCachePath value:value];
}

- (void)getCacheDataForKey:(NSString *)key inPath:(NSString *)path value:(HUCacheValueBlock)value{
    if (!key)return;
    NSData *obj = [self.memoryCache objectForKey:key];
    if (obj) {
        if (value) {
...
- (void)automaticCleanCache{
   [self clearCacheWithTime:hu_defaultCacheMaxCacheAge completion:nil];
}

- (void)clearCacheWithTime:(NSTimeInterval)time completion:(HUCacheCompletedBlock)completion{
     [self clearCacheWithTime:time inPath:self.diskCachePath completion:completion];
}

- (void)clearCacheWithTime:(NSTimeInterval)time inPath:(NSString *)path completion:(HUCacheCompletedBlock)completion{
    if (!time||!path)return;
    dispatch_async(self.operationQueue,^{
        // “-” time
...
- (void)backgroundCleanCacheWithPath:(NSString *)path{
    ...
    [self clearCacheWithTime:hu_defaultCacheMaxCacheAge inPath:path completion:^{
        [application endBackgroundTask:bgTask];
        bgTask = UIBackgroundTaskInvalid;
    }];
}
...
- (void)clearCacheForkey:(NSString *)key completion:(HUCacheCompletedBlock)completion{
    [self clearCacheForkey:key inPath:self.diskCachePath completion:completion];
}

- (void)clearCacheForkey:(NSString *)key inPath:(NSString *)path completion:(HUCacheCompletedBlock)completion{
    if (!key)return;
...
- (void)clearCacheForkey:(NSString *)key time:(NSTimeInterval)time completion:(HUCacheCompletedBlock)completion{
    [self clearCacheForkey:key time:time inPath:self.diskCachePath completion:completion];
}

- (void)clearCacheForkey:(NSString *)key time:(NSTimeInterval)time inPath:(NSString *)path completion:(HUCacheCompletedBlock)completion{
...
- (void)clearCacheOnCompletion:(HUCacheCompletedBlock)completion{
    dispatch_async(self.operationQueue, ^{
...
- (void)clearDiskWithPath:(NSString *)path completion:(HUCacheCompletedBlock)completion{
    if (!path)return;
     dispatch_async(self.operationQueue, ^{
         NSDirectoryEnumerator *fileEnumerator = [[NSFileManager defaultManager] enumeratorAtPath:path];
         for (NSString *fileName in fileEnumerator){
